...
enum tiny_ErrorCode tiny_ConstrainedBackwardPass(tiny_AdmmWorkspace* work) {
  ...
  if (work->stgs->en_cstr_goal) {
    ...
    slap_MatMulAdd(work->soln->P[N - 1], slap_Transpose(work->Qxx), work->Qxx, work->rho, 1);  // P[N]  += ρ*H'H
  }
  ...
  if (work->stgs->en_cstr_states) {
    ...
    slap_ScaleByConst(work->cx_mask, work->rho);  // mask = ρ*mask
  ...
  if (model[0].ltv && model[0].affine) {
    for (int k = N - 2; k >= 0; --k) {
      ...
      if (work->stgs->en_cstr_inputs) {
        ...
        slap_ScaleByConst(work->cu_mask, work->rho);  // mask = ρ*mask
      ...
      slap_Copy(work->soln->Kinf, work->Qux);
      slap_Copy(work->soln->d[k], work->Qu);
      slap_CholeskySolve(Quu_temp, work->soln->d[k]);  // d = Quu\Qu
      slap_CholeskySolve(Quu_temp, work->soln->Kinf);  // K = Quu\Qux

      // Cost-to-Go Hessian: P = Qxx + K'Quu*K - K'Qux - Qux'K
      slap_Copy(work->soln->P[k], work->Qxx);                                  // P = Qxx
      slap_MatMulAdd(work->Qxu, slap_Transpose(work->soln->Kinf), work->Quu, 1, 0);  // Qxu = K'Quu
      slap_MatMulAdd(work->soln->P[k], work->Qxu, work->soln->Kinf, 1, 1);           // P += K'Quu*K
      slap_MatMulAdd(work->soln->P[k], slap_Transpose(work->soln->Kinf), work->Qux, -2,
                    1);  // P -= K'Qux
      // slap_MatMulAdd(work->soln->P[k], slap_Transpose(Qux), work->soln->Kinf, -1,
      //                1);  // P -= Qux'K
      ...
      slap_MatMulAdd(work->soln->p[k], slap_Transpose(work->soln->Kinf), work->Qu, -1,
                    1);  // p -= K'Qu
...
enum tiny_ErrorCode tiny_SolveAlLqr(tiny_AdmmWorkspace* work) {
  ...
  for (work->info->iter_al = 1; work->info->iter_al <= work->stgs->max_iter;
       ++work->info->iter_al) {
    ...
    if (verbose > 0) {
      ...
      printf("%7d%12.2e%12.2e%12.2e%10.1e%10.1e\n", 
            work->info->iter_al, work->info->obj_val, work->info->pri_res,
            work->info->dua_res, work->reg, work->rho);
    }
...
int tiny_CheckAl(tiny_AdmmWorkspace* work) {
  ...
  if (work->info->dua_res < work->stgs->tol_abs_dual) {
